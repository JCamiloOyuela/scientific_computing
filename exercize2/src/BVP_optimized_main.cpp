...
namespace BVP {
  ...
  public:
    ...
    real_type F( real_type x, integer i ) const override {
      switch ( i ) {
        case 0: return (pi*pi + 2) * sin(pi*x)   - sin(2*pi*x);
        case 1: return (4*pi*pi + 2) * sin(2*pi*x) - sin(pi*x);
      }
  ...
  run_experiment( BVP_Problem_base const & problem, integer N ) {
    ...
    for ( integer blk = 0; blk < N; ++blk ) {
      ...
      for ( integer i = 0; i < n; ++i ) {
        ...
        if ( blk == 0 )   system.b( blk, i ) += problem.left_BC(i)  * inv_h2;
        if ( blk == N-1 ) system.b( blk, i ) += problem.right_BC(i) * inv_h2;
...
main( int argc, char ** argv ) {
  ...
  std::ofstream csv("results_optimized.csv");
