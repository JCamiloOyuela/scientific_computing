...
namespace BVP {
  ...
  void Block_Tridiagonal::solve() {
    ...
    mat_type D_hat = diagonal_block_view(0);
    LU_type  lu( D_hat );

    if ( _n_blk > 1 ) U_hat[0] = lu.solve( upper_block_view(0) );
    rhs_hat[0] = lu.solve( rhs_block_view(0) );
    ...
    for ( integer blk = 1; blk < _n_blk; ++blk ) {
      // D̂ᵢ = Dᵢ - Lᵢ₋₁ Ûᵢ₋₁.
      D_hat = diagonal_block_view(blk);
      D_hat.noalias() -= lower_block_view(blk-1) * U_hat[blk-1];
      lu.compute( D_hat );
      if ( !lu.isInvertible() ) {
      ...
      if ( blk < _n_blk-1 ) U_hat[blk] = lu.solve( upper_block_view(blk) );

      // r̂ᵢ = D̂ᵢ⁻¹ ( bᵢ - Lᵢ₋₁ r̂ᵢ₋₁ ).
      vec_type rhs_tmp = rhs_block_view(blk);
      rhs_tmp.noalias() -= lower_block_view(blk-1) * rhs_hat[blk-1];
      rhs_hat[blk] = lu.solve( rhs_tmp );
    }

    // Backward substitution:
    //   x_{m-1} = r̂_{m-1},
    //   x_i     = r̂_i - Û_i x_{i+1}.
    solution_block_view(_n_blk-1) = rhs_hat[_n_blk-1];

    for ( integer blk = _n_blk-2; blk >= 0; --blk ) {
      solution_block_view(blk) = rhs_hat[blk] - U_hat[blk] * solution_block_view(blk+1);
