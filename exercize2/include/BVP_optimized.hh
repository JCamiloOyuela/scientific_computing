...
/**
 * @file BVP.hh
 * @brief Data structures and interfaces for a 1D vector boundary value problem.
 *
 * This file contains:
...
namespace BVP {
  ...
  public:
    ...
    /// @brief Read-only view of diagonal block `blk`.
    auto diagonal_block_view( integer blk ) const {
      return D_bloks.middleCols( blk*_n, _n );
    }

    /// @brief Read-only view of lower block `blk`.
    auto lower_block_view( integer blk ) const {return L_bloks.middleCols( blk*_n, _n );}

    /// @brief Read-only view of upper block `blk`.
    auto upper_block_view( integer blk ) const {return U_bloks.middleCols( blk*_n, _n );}

    /// @brief Read-only view of right-hand side block `blk`.
    auto rhs_block_view( integer blk ) const {return B_bloks.segment( blk*_n, _n );}

    /// @brief Writable view of solution block `blk`.
    auto solution_block_view( integer blk ) {return X_bloks.segment( blk*_n, _n );}

    /// @brief Read-only view of solution block `blk`.
    auto solution_block_view( integer blk ) const {return X_bloks.segment( blk*_n, _n );}
